...
case class Matrix(data : List[List[Int]]) {
    ...
    def saddlePoints() : Set[(Int, Int)] =  {
        ...
        def maxes : List[Element] = data.zipWithIndex.flatMap(elmInRow(_, (_ > _)));
        
        // first transpose the matrix, then swap back rows with columns
        def mins : List[Element] = data.transpose.zipWithIndex.flatMap(elmInRow(_, (_ < _))).map( (elm : Element) => (elm._1.swap,elm._2));
...
object Matrix {
    def apply(data : List[List[Int]]) = new Matrix(data) 
}